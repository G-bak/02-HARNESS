...
Generator 실행이 끝난 뒤에는 `allowed_context.target_files`와 실제 변경 파일도 대조한다. wrapper는 실행 직전 워킹트리 변경 파일 목록을 baseline으로 잡고, 실행 후 새로 생긴 변경분만 검사한다. handoff 운영 산출물(`generator-result.json`, `generator-stdout.raw.json`, `generator-stderr.log`, `generator-run.json`, 해당 Task 원장)을 제외한 신규 변경 파일이 `target_files` 밖에 있으면 `HOLD`로 막는다. 따라서 Generator가 새 파일을 만들 가능성이 있으면 그 파일도 handoff의 `target_files`에 미리 넣어야 한다.

> ⚠ **Known gotcha (TASK-20260429-015 출처)** — wrapper가 `main...HEAD` 전체 branch diff를 검사하면, Generator 실행 전에 만든 handoff·spec·input 같은 준비 파일까지 Generator 변경으로 오판할 수 있다. 실제 범위 검증은 현재 실행에서 새로 생긴 변경분 기준이어야 하며, `generator-run.json`의 `pre_run_changed_files`, `changed_files`, `working_tree_files`를 함께 확인한다.
...
다만 CLI가 stdout을 `{ "result": "..." }` 같은 wrapper JSON으로 감쌀 수 있으므로, wrapper는 `generator-stdout.raw.json`을 먼저 저장한 뒤 실제 Generator JSON을 추출·정규화해서 `generator-result.json`에 쓴다. Claude CLI가 `structured_output`을 함께 제공하면 `result` 자연어보다 `structured_output`을 우선한다. `result`가 자연어뿐이면 wrapper는 실행 직전 baseline 이후의 `allowed_context.target_files` 변경분을 확인하고, 대상 파일 변경이 있을 때만 합성 `generator-result.json`을 만들며 `generator-run.json`에 `pre_run_changed_files`, `changed_files`, `working_tree_files`, `diff_summary`를 남긴다.

자연어 fallback은 성공을 추정하는 우회 경로가 아니다. 대상 파일 diff가 없거나 변경 범위가 `allowed_context.target_files` 밖이면 Task는 `HOLD`로 남기고, wrapper는 no-diff 사유가 담긴 합성 결과만 감사용으로 기록한다.
...
이 검증을 통과하거나 자연어 fallback에서 target file diff 검증을 통과해야만 `PENDING_VALIDATION`으로 기록한다. JSON 파싱, 필수 필드 검증, 자연어 fallback의 diff 검증 중 하나라도 실패하면 Task는 `HOLD`로 남기고, Generator 출력 형식 또는 실제 변경 상태를 고친 뒤 재실행한다.
