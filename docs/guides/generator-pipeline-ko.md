...
다만 CLI가 stdout을 `{ "result": "..." }` 같은 wrapper JSON으로 감쌀 수 있으므로, wrapper는 `generator-stdout.raw.json`을 먼저 저장한 뒤 실제 Generator JSON을 추출·정규화해서 `generator-result.json`에 쓴다. Claude CLI가 `structured_output`을 함께 제공하면 `result` 자연어보다 `structured_output`을 우선한다. `result`가 자연어뿐이면 wrapper는 `allowed_context.target_files`에 대한 git diff를 확인하고, 대상 파일 변경이 있을 때만 합성 `generator-result.json`을 만들며 `generator-run.json`에 `changed_files`와 `diff_summary`를 남긴다.
