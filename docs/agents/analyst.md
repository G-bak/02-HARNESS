...
7. **Tier 1/2 창작 산출물(이미지·광고 카피·운영 가이드 한 줄 등)을 직접 생성하거나 외부 CLI(예: Codex CLI built-in `image_gen`)를 직접 호출해 산출한다.** 같은 외부 CLI invocation에서 검색이 함께 일어나면 검색 부분은 Researcher 활동으로 분리 기록하고, 생성·산출 부분은 Analyst 활동으로 귀속한다. Tier 2/3 제품 코드·구조적 변경은 Generator를 거쳐야 한다.
