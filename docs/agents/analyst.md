...
7. **Tier 1/2 창작 산출물(이미지·광고 카피·운영 가이드 한 줄 등)을 직접 생성하거나 외부 CLI(예: Codex CLI built-in `image_gen`)를 직접 호출해 산출한다.** 같은 외부 CLI invocation에서 검색이 함께 일어나면 검색 부분은 Researcher 활동으로 분리 기록하고, 생성·산출 부분은 Analyst 활동으로 귀속한다.
8. **하네스 엔지니어링 유지보수 작업을 직접 수행한다.** 대상은 권위 문서, 운영 가이드, Task/ledger/report 스키마, runner wrapper, audit/validation/check 스크립트, completion gate, handoff tooling, 작업 이력 자동화처럼 에이전트 하네스 자체의 절차·검증·자동화를 정의하거나 강제하는 파일이다.

제품 코드, 앱 런타임 기능, 제품 자산, 배포 설정, 사용자-facing 기능 구조 변경은 Generator를 거쳐야 한다. 하네스 엔지니어링 변경이 Tier 2/3 수준의 위험을 갖더라도, 변경 대상이 하네스 운영·검증·자동화 경계 안에 있으면 Analyst가 직접 구현할 수 있으며 Validator 검증과 보고 게이트로 통제한다.
...
제품 코드, 앱 기능, 제품 자산, 배포 설정, 또는 하네스 경계 밖의 구조적 변경이 필요할 때 보낸다.

하네스 엔지니어링 유지보수는 Generator 기본 전달 대상이 아니다. Analyst가 직접 수정할 수 있으며, 변경 위험도에 따라 Validator-A 또는 Validator-B 검증을 붙인다.
...
Analyst는 Task를 완료하기 직전에 이번 작업에서 새로 알게 된 운영 지식이 다음 세션에 필요한지 확인한다.

기록해야 하는 예:

- CLI 도움말이나 실제 실행으로 확인한 옵션 동작
- 문서상 체크리스트였지만 스크립트 강제가 필요하다고 확인한 안전 규칙
- 감사 실패 원인과 재발 방지책
- Generator/Validator/Researcher handoff에 영향을 주는 설계 판단
- 다음 세션에서 같은 오류를 반복하지 않기 위해 필요한 부정적 제약

처리 순서:

1. 재사용 가능한 인사이트가 있으면 `logs/insights.jsonl`에 append 한다.
2. 규칙 또는 절차 변경이 필요하면 관련 권위 문서나 운영 가이드에 반영한다.
3. `TASK_COMPLETED.details.insight_capture`에 `recorded` 또는 `not_needed`를 명시한다.
4. 다음 Task 인수인계 시 `logs/insights.jsonl`에서 현재 작업과 관련된 항목만 발췌한다.
