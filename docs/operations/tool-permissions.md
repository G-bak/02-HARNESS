...
| **이미지 생성 도구** | ✅ (Tier 1/2 창작 산출물 한정 — Codex `image_gen` 등) | ❌ | ✅ (제품 코드 변경에 자산 필요한 경우 task/* 한정) | ❌ | ❌ |

> ⚠ **이미지 생성 권한 분리 (TASK-20260429-021 정정).** Researcher는 이미지·코드·문서 생성 권한이 없다. 같은 외부 CLI invocation(예: Codex CLI)에서 웹 검색과 이미지 생성을 함께 수행하더라도 권한 귀속은 분리된다 — 검색은 Researcher, 이미지 산출은 Analyst(Tier 1/2 창작) 또는 Generator(Tier 2/3 제품 자산). 보고서·원장에서 활동을 분리해 기록한다.
